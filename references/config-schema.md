...
category は LLM がフォームラベルから判断する意味識別子。表記ゆれを吸収した統一値を設定する。
主要値: `department`(部署), `position`(役職), `inquiry_type`(問い合わせ種別), `company_url`(会社URL), `employee_count`(従業員数), `how_found`(きっかけ), `other`(その他)
