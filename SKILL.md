...
失敗履歴の蓄積ファイル。セッションごとに上書きせず追記・マージする。実行前に読み込み、ループ終了後に保存する。
スキーマ詳細は `references/config-schema.md` を参照。ファイルが存在しない場合は `{ "entries": [] }` として扱う。

### `docs/error-rules.json`

`error-log.json` で3回以上発生したエラーパターンから昇格した自動スキップルール。フォームURL遷移前に参照する。
スキーマ詳細は `references/config-schema.md` を参照。ファイルが存在しない場合は `{ "rules": [] }` として扱う。
...
`docs/progress.json` の `results` を集計し、以下の形式で出力する：
...
---
エラールール昇格候補（3回以上発生）:
- イプソス株式会社: CAPTCHA検出（3回）→ 自動スキップに追加済み ／ スキップ済み
---
...
エラーは **リトライ対象** と **即スキップ** に分類される。リトライ対象のエラーはステップ 13 のリトライルール（最大3回）に従い、即スキップのエラーはリトライせず次の企業に進む。
...
- 送信後にCAPTCHAエラーが返された場合: ステップ 13（リトライ＆エラー時の処理）に進む。3回リトライしても失敗した場合は「CAPTCHA検出（reCAPTCHA v3）」としてエラーリストに記録する
...
→ ステップ 13（リトライ＆エラー時の処理）に進む。3回リトライしても失敗した場合は「タイムアウト」としてエラーリストに記録する
...
→ ステップ 13（リトライ＆エラー時の処理）に進む。3回リトライしても失敗した場合は「送信エラー: {エラーメッセージ内容}」としてエラーリストに記録する
